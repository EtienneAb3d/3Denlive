...
public class DepthmapStereo extends Processor {
	class Key{
		double d = 0;
		double x = 0;
		double p = 0;
		...
		Clip depthmapClipMedia = null;
	}
	Vector<Key> keys = new Vector<Key>();
	
	public DepthmapStereo(TDenlive aTDe,Media aTargetMedia) {
	...
	@Override
	public String getClassName() {return "DepthmapStereo";}
	...
	public int openProject(Vector<XmlObject> aOs, int o) throws Exception {
		...
		for(;o < aOs.size();o++) {
			...
			if(aO instanceof XmlTag) {
				...
				if("depthmapClipMedia".equalsIgnoreCase(aT.tagName)) {
					o++;
					long aMediaId = Long.parseLong(aOs.elementAt(o).text);
					if(aMediaId >= 0) {
						keys.lastElement().depthmapClipMedia = ((Clip)targetMedia).addPairedClip(tde.mediaList.getMediaByUID(aMediaId), 1);
					}
				}
			}
		}
	...
	public void saveProject(StringBuffer aSB) throws Exception {
		...
		for(Key aK : keys) {
			...
			aSB.append("							<depthmapClipMedia>" + (aK.depthmapClipMedia == null ? -1 : aK.depthmapClipMedia.getMedia().UID) + "</depthmapClipMedia>\n");
		}
		aSB.append("						</DepthmapStereo>\n");
	}
	...
	@Override
	void setDescr() {
		name = tde.gui.lngs.get("Processor.DepthmapStereo");
...
//		return true; 
	...
	public void setDepthmapClipMedia(int aKey,String aUID) {
		warnNextAlignments();
		if(aUID.trim().isEmpty()) {
			if(keys.elementAt(aKey).depthmapClipMedia != null) {
				((Clip)targetMedia).removePairedClip(keys.elementAt(aKey).depthmapClipMedia,keys.elementAt(aKey).depthmapClipMedia.pairedType);
			}
			keys.elementAt(aKey).depthmapClipMedia = null;
			return;
		}
		Media aMedia = tde.mediaList.getMediaByUID(Long.parseLong(aUID));
		if(aMedia == null) {
			//??
			if(keys.elementAt(aKey).depthmapClipMedia != null) {
				((Clip)targetMedia).removePairedClip(keys.elementAt(aKey).depthmapClipMedia,keys.elementAt(aKey).depthmapClipMedia.pairedType);
		...
		keys.elementAt(aKey).depthmapClipMedia = ((Clip)targetMedia).addPairedClip(aMedia, 1);
	}
	
	public Clip getDepthmapClipMedia(int aKey) {
		return keys.elementAt(aKey).depthmapClipMedia;
	}
		...
		for(int k = keys.size()-1;k >= 0;k--) {
			...
			if(aKey.time <= aTime) {
				if(k == keys.size()-1) {
					...
					aInter.depthmapClipMedia = aKey.depthmapClipMedia;
					return aInter;
				}
	...
	public void process2Awt(Media aMedia,int aProcessingMode,long aTime) {
		...
		try {
			Key aKey = getInterKeyGT(aTime);
			if(aKey.depthmapClipMedia == null) {
				return;
			}
			BufferedImage aBIDML = aKey.depthmapClipMedia.getWorkLeft(aProcessingMode,aTime);
			BufferedImage aBIDMR = aKey.depthmapClipMedia.getWorkRight(aProcessingMode,aTime);
			...
			Thread aThL = new Thread(new Runnable() {
				...
				public void run() {
					BufferedImage aProcessedBIL = 
							...
							processAwtHdr(aBIL,aBIDML,aProcessingMode,aTime,true):
								processAwtImage(aBIL,aBIDML,aProcessingMode,aTime,true);
			...
			Thread aThR = new Thread(new Runnable() {
				...
				public void run() {
					BufferedImage aProcessedBIR = 
							tde.config.processingHdr[aProcessingMode]?
									processAwtHdr(aBIR,aBIDMR,aProcessingMode,aTime,false):
										processAwtImage(aBIR,aBIDMR,aProcessingMode,aTime,false);
	...
	BufferedImage processAwtImage(BufferedImage aBI,BufferedImage aDM,int aProcessingMode,long aTime,boolean aIsL) {
		...
		for(int y = 0;y<aHeight;y++) {
			...
			for(int x = 0;x<aWidth;x++) {
				...
				double aZ = (aKey.blackIsFront?1:-1)*((((aDMR+aDMG+aDMB)/(3.0*255))-aMin)/(aMax-aMin)-0.5);
				double aDx = -(int)(((aIsL?-1:+1)*(aKey.d*aZ)/4.0+aKey.x*aZ)
						*aWidth)
						;
				...
				if(aLastX >= 0 && Math.abs(aLastX-aXd) >= 2 && 
						(Math.abs(aLastX-aXd) <= aHoleThX || Math.abs(aDLineZ[aLastX]-aDLineZ[aXd]) <= aHoleThZ)) {
					int aBegX = Math.min(aLastX,aXd);
					int aEndX = Math.max(aLastX,aXd);
...
//				}
//
//				double aDx = (int)(((aIsL?-1:+1)*(aKey.d*aDMZ)/4.0+aKey.x*aDMZ)
//						*aWidth)
//						;
//				aDx -= aKey.p*aDMZ*(aWidth/2.0 - x);
//				int aXd = (int)(x+aDx);
//
//				if(aXd < 0 || aXd >= aWidth) {
//					//Out
...
//				}
//
//				aFDB[y*aWidth*4+x*4] = aDB[y*aWidth*4+(x+aDx)*4];
//				aFDB[y*aWidth*4+x*4+1] = aDB[y*aWidth*4+(x+aDx)*4+1];
//				aFDB[y*aWidth*4+x*4+2] = aDB[y*aWidth*4+(x+aDx)*4+2];
//				aFDB[y*aWidth*4+x*4+3] = aDB[y*aWidth*4+(x+aDx)*4+3];
//			}
			//Interpolate
			aLastX = -1;
			for(int x = 0;x<aWidth;x++) {
