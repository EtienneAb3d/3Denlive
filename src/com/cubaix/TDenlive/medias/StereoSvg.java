...
import com.cubaix.TDenlive.salamander.SvgRasterizer;
import com.cubaix.TDenlive.xml.XmlObject;

public class StereoSvg extends StereoImage {
	SvgRasterizer leftReader = null;
	SvgRasterizer rightReader = null;
	
	public StereoSvg(TDenlive aTDe) {
		super(aTDe);
	}

	@Override
	public int openProject(Vector<XmlObject> aOs, int o) throws Exception {
		return super.openProject(aOs, o,"StereoSvg");
	}

	@Override
	public void saveProject(StringBuffer aSB) throws Exception {
		super.saveProject(aSB,"StereoSvg");
	}
	...
	BufferedImage loadImage(String aPath,int aProcessingMode,long aTime,boolean aIsLeft) throws Exception {
		...
		if(aIsLeft) {
			if(leftReader == null) {
				leftReader = new SvgRasterizer(tde,aPath,null);
			}
			aBI = leftReader.getImage(aProcessingMode,aTime);
		}
		else if(!aIsLeft) {
			if(rightReader == null) {
				rightReader = new SvgRasterizer(tde,aPath,leftReader);
			}
